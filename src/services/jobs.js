        ...
        .then((res) => {
          ...
          resolve(
            ResponseUtil.createResponse(
              true,
              200,
              data && data.length === 0 ? "No Jobs found" : "Jobs found",
              data
            )
          );
        })
...
const createJob = (jobData) =>
  new Promise((resolve) => {
    ...
    if (!isValid)
      return resolve(
        ResponseUtil.createResponse(false, 400, "Wrong input", errors)
      );

    Job.create(jobData)
      .then((job) => {
        resolve(ResponseUtil.createResponse(true, 201, "Job Created", job));
      })
      .catch((err) => {
        console.log(err);
